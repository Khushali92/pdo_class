...
require_once('baseclasses/master_attribute_value.php');

class masterAttributeValueChild extends master_attribute_value
{ 	// class : begin
...
function masterAttributeValueChild()
{
$this->master_attribute_value();
}
...
$query = "SELECT $this->selectColumn FROM master_attribute_value WHERE ";
$condition= $this->condition;
$param = $this->param;
	if (is_array($condition)) 
	{
...
$this->id = "" ;
	try
	{
		...
		$sql = $this->conn->prepare("INSERT INTO master_attribute_value (" . implode(", ", $columnNames) . ") VALUES " . implode(", ", $rowsSQL));
