...
function select()
{
$query = "SELECT * FROM technology WHERE ";
...
try
{
	$sql = $this->conn->prepare("SELECT * FROM technology");
	//echo "<pre>";print_r($sql);
	$sql->setFetchMode(PDO::FETCH_ASSOC);
	$sql->execute();
...
function delete()
{
	try
	{	
		$sql =  $this->conn->prepare("DELETE FROM technology  WHERE $this->condition");
		//echo "<pre>";print_r($sql);
		$result = $sql->execute();
		$this->numRows = $sql->rowCount();
		return $result;
...
$columnValue = implode(',',$valueClause);
	try
	{	
		$sql = $this->conn->prepare("INSERT INTO technology ($columnName ) VALUES ( $tempColumnValues )");
...
$setClause = array();
	...
	try
	{	
		$sql = $this->conn->prepare("UPDATE technology SET  $columnName WHERE $this->condition ");
		//echo "<pre>";print_r($sql); print_r($valueClause);
		$result = $sql->execute($valueClause);
